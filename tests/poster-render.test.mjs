...
import { mixedFactsCard, oneWindowCard, twoWindowCard } from "./fixtures/cards.mjs";
...
// 紙面の挙動は合成カード（tests/fixtures/cards.mjs）で検査する。
// 実データを固定具にすると、毎日の巡回で告知が入れ替わるたびに落ちる。
const posterOf = (source, now, options) =>
  renderPosterHtml(posterModel(source, new Date(now), options));

test("有効期限を紙面に必ず刷る", () => {
  // 画面は開くたびに期限を評価し直せるが、紙は刷った瞬間に凍る。
  // いつまで信じてよい紙かが書いていなければ、読む人には古さを判定する手段が無い。
  const html = posterOf(oneWindowCard, "2026-08-01T10:00:00+09:00");
...
test("その日限りの答えは、期限を過ぎた紙に1件も刷らない", () => {
  const inTime = posterOf(oneWindowCard, "2026-08-01T10:00:00+09:00");
  for (const item of oneWindowCard.facts[0].items) {
    assert.ok(inTime.includes(item), `期限内の紙に当日限りの答えが出ていない: ${item}`);
  }

  const expired = posterOf(oneWindowCard, "2026-08-01T20:00:00+09:00", { allowExpired: true });
  for (const item of oneWindowCard.facts[0].items) {
    assert.ok(
...
test("期限を過ぎた紙に残るのは、日付に依存しないと型が保証している答えだけ", () => {
  ...
  // 書かれていることがあり（「南区5か所・西区1か所」など）、型はそれを区別できない。
  // 区別できないものを失効後の紙に残さない。
  const expired = posterOf(mixedFactsCard, "2026-08-01T20:00:00+09:00", { allowExpired: true });

  assert.ok(!expired.includes(mixedFactsCard.summary), "期限切れの紙に summary が残っている");
  for (const step of mixedFactsCard.steps) {
    assert.ok(!expired.includes(step), `期限切れの紙に手順が残っている: ${step}`);
  }
  for (const order of mixedFactsCard.irreversibleOrder) {
    assert.ok(!expired.includes(order), `期限切れの紙に手続きの順番が残っている: ${order}`);
  }
  // 日付つきの臨時窓口は消え、日付に依存しない問い合わせ先は残る。
  // 情報が古くなったときほど「どこへ聞けばよいか」が必要になる。
  assert.ok(!expired.includes("開設窓口：合成窓口"));
  assert.match(expired, /合成課 000-000-0000/);
});
...
test("受付時間帯は枠のまま刷り、いま開いているかを紙に断定させない", () => {
  ...
  const html = posterOf(twoWindowCard, "2026-08-01T09:30:00+09:00");
...
test("出典と確認時刻を紙面に必ず刷る", () => {
  const html = posterOf(oneWindowCard, "2026-08-01T10:00:00+09:00");
  assert.match(html, /出典 合成出典/);
  assert.match(html, /https:\/\/example\.invalid\/fixture/);
  assert.match(html, /開いたページで「合成の応急給水の告知」/);
  assert.match(html, /出典の更新 8月1日\(土\) 07:00／確認 8月1日\(土\) 08:00/);
...
test("期限切れのカードは、明示しない限り刷らずに止める", () => {
  assert.throws(
    () => posterModel(oneWindowCard, new Date("2026-08-01T20:00:00+09:00")),
    ExpiredCardError,
  );
  assert.doesNotThrow(() =>
    posterModel(oneWindowCard, new Date("2026-08-01T20:00:00+09:00"), {
