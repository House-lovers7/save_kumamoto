...
1. 公式情報の再確認 — **未達**
   - 2026-07-31 20:16 JST の巡回時点で、**14カード中3カード**（食料・生活用品／トイレ／
     乳幼児）の公式案内を確認できず、`sourceStatus: "unavailable"` としている。
     画面には「公式の案内を確認できていません」と、何が確認できていないかを出している。
     被災者が最も困る領域が3つ空いた状態では一般公開しない。
   - **燃料は解消した**。資源エネルギー庁「住民拠点SS等検索」が令和8年熊本地震（震度7）を
     現在の災害情報として稼働させており、給油所の営業状況を**営業可／営業不可／確認中**で
     絞り込める。3区分は出典が実際に使っている区分なので、画面でも必ず出す。
   - **巡回の判定ルールを2段階で厳しくした**。まず「リンクが生きているか」から
     「そのページにカードの話題があるか」へ（18:30、6カードが該当）。次に
     「**カードの手順が1つずつリンク先で実行できるか**」へ（20:16、さらに4カードが該当）。
     medical / shelter / communication / roads は、リンクも話題も生きていたが手順が
     実行できなかったため、手順と説明を出典に合わせて書き直した。とくに shelter は
     出典に無い避難所の利用条件一覧を説明文が約束しており、`unavailable` 表示より
     危険な誤認だった（`docs/OPERATIONS.md` 第5章）。
