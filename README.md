...
1. 公式情報の再確認
   - 2026-07-31 に公式ページを再確認し、**全14カードの鮮度を更新した**
     （`checkedAt`/`expiresAt` を更新。期限は確認から24時間、詳細は `docs/OPERATIONS.md` 第5章）。
   - medical カードは出典の県トップページが緊急災害用ページへ差し替わっていたため、
     同日 15:57 JST に導線を「医療機関等を受診される避難者の皆様へ」
     （国保・高齢者医療課）へ再選定し、掲載を実確認したうえで更新した。
   - 24時間期限のため、**毎日1回の再確認巡回が継続する限りにおいて**この項目は解消とする。
     巡回が止まれば全14カードが再び期限切れに戻る。古い静的時刻のまま公開しない。
2. 運営・訂正体制
