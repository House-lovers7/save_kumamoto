...
1. 公式情報の再確認 — **未達**
   - 2026-08-01 09:30 JST の巡回時点で、**熊本市の3カード**（食料・生活用品／トイレ／
     乳幼児）の公式案内を確認できず、`sourceStatus: "unavailable"` としている。
     画面には「公式の案内を確認できていません」と、何が確認できていないかを出している。
     被災者が最も困る領域が3つ空いた状態では一般公開しない。
   - **氷川町では食料配布が公式に出ていた**。「8月1日（土曜日）の水・食料品の配布について」
     （時間と場所つき）で、No-Go #1 で探し続けていた「受け取る側の場所と時間」が
     公式に出ている唯一の例。カードを追加した（`food-hikawa`）。
     **熊本市には無い**ことは、被災者支援制度の冊子PDFの目次まで当たって確認している
     （生活必需品の支給は申請受付前で、支給品一覧も前回災害時の内容と明記されている）。
