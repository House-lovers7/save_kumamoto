...
1. 公式情報の再確認
   - 2026-07-31 に公式ページ（6URL）を再確認し、13/14カードの鮮度を更新した
     （`checkedAt`/`expiresAt` を更新。期限は確認から24時間、詳細は `docs/OPERATIONS.md` 第5章）。
   - medical カード1件は、出典の県トップページが緊急災害用ページへ差し替わっており
     医療導線を確認できなかったため未更新（失効表示のまま）。医療導線の再選定が残課題。
   - 24時間期限のため、**毎日1回の再確認巡回が継続する限りにおいて**この項目は解消とする。
     巡回が止まれば13カードも再び期限切れに戻る。古い静的時刻のまま公開しない。
2. 運営・訂正体制
