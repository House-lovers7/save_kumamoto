...
| Web内部品質 | typecheck、production build、契約テスト、依存監査 | [高] PASS。テスト21件、脆弱性0件（依存監査は2026年7月30日時点） |
...
1. 全カードの公式情報と時刻を公開直前に再確認する。2026-07-31 に公式ページを再確認し、
   **全14カードの鮮度を更新済み**（期限は確認から24時間）。medical カードは県トップの
   災害ページ化に伴い導線を「医療機関等を受診される避難者の皆様へ」へ再選定した
   （15:57 JST 実確認）。24時間期限のため、**毎日1回の再確認巡回が継続する限りにおいて**解消。
