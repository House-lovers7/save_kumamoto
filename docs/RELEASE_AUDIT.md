...
1. 全カードの公式情報と時刻を公開直前に再確認する。2026-07-31 に公式ページ（6URL）を再確認し、
   **13/14カードの鮮度を更新済み**（期限は確認から24時間）。medical カード1件は医療導線が
   未確認のため失効表示のまま。24時間期限のため、**毎日1回の再確認巡回が継続する限りにおいて**解消。
