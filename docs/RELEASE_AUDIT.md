...
| Link | 必要な証拠 | 現在の状態 |
|---|---|---|
| 公式情報 | 公式ページ、出典URL、発表・取得・確認・失効時刻 | [高] 2026年7月31日 20:16 JST に公式ページ12件（重複除く）を再確認し、全件がHTTP 200。**6カードは掲載継続、2カードは導線を再選定（fuel / roads）、3カードは出典を維持して手順・説明を実態へ修正（medical / shelter / communication）、3カードは公式の該当案内を確認できず `unavailable`**。判定は「カードの手順が1つずつリンク先で実行できるか」（`docs/OPERATIONS.md` 第5章） |
...
1. 全カードの公式情報と時刻を公開直前に再確認する。**未達**。
   2026-07-31 20:16 JST の巡回時点で、14カード中**3カード**（食料・生活用品／トイレ／
   乳幼児）の公式案内を確認できず `sourceStatus: "unavailable"` としている。画面には
   「公式の案内を確認できていません」と何が確認できていないかを出しているが、
   被災者が最も困る領域が3つ空いた状態での一般公開はしない。
   燃料は資源エネルギー庁「住民拠点SS等検索」への再選定で解消した。
   24時間期限のため、**毎日1回の再確認巡回が継続する限りにおいて**残り11カードは有効。
