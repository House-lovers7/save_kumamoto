...
- 実ブラウザ（headless Chrome、390×844）：
  - 初期HTMLに答えが出ている（応急給水所の名称・住所、無料入浴の実施施設と店休日、問い合わせ先の電話番号、り災証明の窓口、住家の緊急の修理の完了期限）。件数は巡回ごとに動くので固定しない
...
- **外した理由**: 探索導線は枯渇するまで再探索済みで（`docs/OPERATIONS.md` 第5章、同じ導線の再探索は禁止）、これ以上は開発・運用側の作業では埋まらず、公式が案内を出すのを待つしかない。「未確認」を隠さず明示することは、本アプリが唯一守ると約束した品質基準（鮮度と失効を正直に出す）の適用であって違反ではない。確認済みカード（応急給水・無料入浴・り災証明ほか）を届けないことの不利益が上回ると判断した。
