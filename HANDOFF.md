...
## 未決の設計論点：キャラクター／アイコン（2026-07-31 ユーザー提案）

ユーザーから「キャラクターアイコンを入れた方が UI/UX がよくなる。くまモンは入れられるか」
という提案があった。**くまモンは No-Go** と整理した。次セッションで代替案を判断する。

### くまモンを使わない理由

1. **権利**[中] — 熊本県が権利を持つ商標で、イラスト利用には県への利用許諾申請と許諾が必須。
   原則無償だが無断使用は不可、デザイン改変も禁止。申請窓口・現行要綱は未確認なので、
   仮に検討するなら熊本県公式ページの一次情報確認が要る（External Specification Preflight）
2. **本アプリの最優先の品質基準と衝突**[高] — README 冒頭で「熊本県、熊本市、NERV その他の
   公的・民間サービスの**公式アプリではありません**」と宣言している。くまモンは県の公式キャラなので、
   置いた瞬間に被災者は「県の公式アプリ」と読む。**発信主体の誤認は、鮮度を偽ることと同じ種類の不誠実**で、
   災害時には実害になる。県から許諾が下りても理由は消えない（むしろ公式に見えるので悪化する）

### 代替案（次セッションで判断）

1. **困りごとグリッドにピクトグラムを追加**（推奨）— 現状の need grid は文字のみ（`index.tsx:276-305`）。
   水・食料・トイレ・医療・避難所などに絵記号が付くと、パニック時・低視力時の到達速度が上がる。
   権利問題なし。被災者価値が最も高い
2. **オリジナルの控えめなアプリアイコン** — 公式らしさを装わないもの。ストア申請時にはどのみち要る
3. くまモン — No-Go（上記）

判断の前提として、災害アプリでキャラクターを前面に出すと緊急時のトーンが軽くなり情報密度も下がる。
119/110 の導線の隣にマスコットを置かない、という制約は維持する。

なお本論点は **No-Go #4（ネイティブ実機検証）の完了より優先度が低い**。
実行境界の証拠がまだ 1 件も無い状態なので、先に Remaining 1〜3 を片づけること。
